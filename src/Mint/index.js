...
cardano-cli transaction build-raw --fee $fee --tx-in $txix --tx-out $address+$output+"$tokenamount $policyid.$tokenname" --mint="$tokenamount $policyid.$tokenname" --minting-script-file policy/policy.script --minting-script-file policy/policy.script --invalid-hereafter $slotnumber --out-file ./transactions/raw/$tokenname.raw`
    ...
    buildSendRawTX(options) {
        let promise = new Promise((resolve, reject) => {
            ...
            let minterOutput = options.sendData.output == undefined ? "0" : options.output - sendFee - 3000000 //options.sendData.mintTXHash.balance.lovelace
...
cardano-cli transaction build-raw --fee ${sendFee} --tx-in ${options.mintWalletInfo.txixhash} --tx-out ${options.customer.address}+3000000+"1 ${options.policy.id.trim()}.${options.request.metadata.asset_id}" --tx-out ${options.mintWalletInfo.address}+${minterOutput} --out-file ./transactions/raw/${options.request.metadata.asset_id}-send.raw`
                exec(cmd , (err, stdout, stderr) => {
                    if (err) {
                        reject(err)
                        return;
