...
cardano-cli transaction build-raw --fee "0" --tx-in "${options.mintWalletInfo.txixhash}" --tx-out '${options.mintWalletInfo.address}+${output}+"${options.request.metadata.amount} ${options.policy.id}.${options.request.metadata.asset_id}"' --mint="${options.metadata.amount} ${options.policy.id}.${options.metadata.asset_id}" `
            console.log(cmd)
